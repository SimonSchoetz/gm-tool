...
Entity vocabulary grounded in the actual database schema. Most terms below own a dedicated `db/<domain>/schema.ts`; NPC, PC, Foe, Faction, Location, and Item are rows of one shared table instead — see the Base entity row. Every entity below is scoped to a single Adventure via `adventure_id` unless noted otherwise.
...
| Base entity                | The building-block subset of entity types — the content (NPC, PC, Foe, Faction, Location, Item) a GM populates an Adventure with, which Session Steps and Encounters draw on. Implemented as one shared `base_entities` table (`db/base-entity/schema.ts`), distinguished by an `entity_type` column (`BaseEntityType`, `domain/entities/entityTypes.ts`). "Base" means building block, not supertype: Encounter, Adventure, and Session are entity types (`ENTITY_TYPES`) but never base entities (`BASE_ENTITY_TYPES`). |
