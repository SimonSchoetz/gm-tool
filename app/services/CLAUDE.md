...
- A service with a single corresponding `@db/<domain>` module is named for that domain's singular directory plus `Service`: `adventureService.ts` (`@db/adventure`), `sessionService.ts` (`@db/session`), `baseEntityService.ts` (`@db/base-entity`). A service spanning multiple domains or wrapping no domain module (e.g. connectivity/sync infrastructure) is named for its feature instead, with no singular/plural constraint.
