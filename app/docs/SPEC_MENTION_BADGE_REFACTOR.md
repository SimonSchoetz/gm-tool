...
### Re-snapshotting persists a rename, never a table-color change

The opportunistic re-snapshot effect (SF8) exists so a mention's deleted-entity fallback shows the entity's last-known name, not the name at insertion time — a legitimate reason to touch a document's persisted state, since a rename is real entity content the user changed. Color is different: `resolvedColor` (`tableConfig?.color ?? color`) is a per-table-config value shared by every mention of that table, not a per-entity fact, and the deleted-entity fallback (`MentionBadge`'s `deleted` branch) never reads `color` or `resolvedColor` at all — it renders with a fixed muted CSS token regardless of what the node's stored color is. The only role `MentionNode.__color` still plays is a last-resort fallback for the rare case `useTableConfigs()` returns no matching `table_name` row. That role does not benefit from staying fresh, and persisting it does real harm: it makes every mention badge write to its owning document — bumping `updated_at` and firing the sync trigger (`db/_migrations/1784365870026_add_sync_infrastructure.ts`) — the first time it renders after a Table Config color edit, touching every document that happens to mention any entity of that table, regardless of whether the user edited that document. The re-snapshot effect therefore only ever calls `MentionNode.setDisplayName()`; `MentionNode.setColor()` is removed — the stored `__color` field is written once, at mention-creation time, and never rewritten thereafter.
