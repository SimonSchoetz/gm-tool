...
**SF self-containment for architectural consequences.** When a KAD produces a structural choice in a specific SF that an implementer reading only that SF file would find surprising — a type from a foreign layer appearing as a parameter, a deliberate deviation from a boundary rule, or any choice whose rationale lives only in the root KAD section — the SF body must include a brief inline rationale note at the decision site, naming the KAD heading it corresponds to. The same inline-note requirement applies when a sub-feature's stated boundary (e.g. "do not modify X") knowingly walls off a violation discovered during spec-writing rather than a KAD-driven choice — the note is prefixed with the literal marker `[DEFERRED-VIOLATION: <rule + location>]` instead of a KAD heading, followed by the reason it is deferred rather than fixed now; the bracketed marker is required verbatim so a downstream scan can find it deterministically, the same role `[MANUAL-VERIFY]` already plays for untested-risk markers. An SF that contains a surprising structural choice, or a scope-excluding boundary concealing a known violation with no `[DEFERRED-VIOLATION]` note, is a spec defect: the implementing instance has no basis to distinguish intentional design or deliberate deferral from error.
...
**Insertion anchor validity.** When a spec instructs the implementer to insert code at a named anchor line within an existing file (e.g., "add directly after `const [x, setX]`"), verify before writing that the anchor line appears after every dependency of the inserted declarations. If the inserted declarations reference a value declared below the proposed anchor, the anchor is wrong — state the correct anchor that appears after the last required dependency. This obligation is not conditional on the spec having named an anchor at all: when the inserted construct consumes a same-scope dependency synchronously — a direct read, or a closure over it invoked before render completes — the spec must state the placement constraint even if no anchor line is otherwise named, since convention (e.g. hooks conventionally sitting at the top of a component body) can pull the implementer toward the wrong position by default. A closure whose invocation is deferred until after render completes (an effect, an event handler, any post-render callback) is not at risk regardless of textual order and does not trigger this obligation. The failure this produces is not uniformly compiler-caught: a direct same-scope read of a not-yet-declared value is a tsc "used before declaration" error, but a synchronously-invoked closure reading a not-yet-declared value is invisible to tsc and throws an uncaught temporal-dead-zone `ReferenceError` at runtime instead. A spec that names an anchor preceding a dependency, or that introduces a synchronously-consumed same-scope dependency with no placement constraint stated at all, is a spec defect: the implementer cannot be expected to diagnose either failure mode from spec silence.
...
Entries whose target is `app/docs/_product/domain-scaffold.md` are the exception to this section's deferred-handoff treatment: `domain-scaffold.md` is not a CLAUDE.md file, is not subject to the direct-edit ban below, and has no `/refine-claude` pathway to act on it — any bullet above that identifies a `domain-scaffold.md` gap (a stale reference, a missing section, an invalidated example) must be resolved by editing `domain-scaffold.md` directly during this spec's own implementation, never logged here as a deferred fact — the implementer follows the spec's layered breakdown, not this file's text, so the edit must be encoded as a concrete item in the relevant sub-feature's Files affected list (`Modified: app/docs/_product/domain-scaffold.md — <what changes>`), not merely resolved as a general intention. For each remaining impact found, state the affected CLAUDE.md file, the fact or structural change driving the update, and its citation per the Epistemological Discipline citation format. If there is no impact, write "None." This section is handoff material for a future `/refine-claude` session — never edit instructions for the implementing instance. Root CLAUDE.md bars direct CLAUDE.md edits by any role, including the instance implementing this spec; phrasing an entry as a directive to apply ("add X to file Y") violates that rule even when the same instance would carry it out. Write each entry as a stated fact plus its consequence (e.g. "`app/src-tauri/CLAUDE.md`'s Dependencies list is missing `tokio` and `futures-core`, both required by SF1's stream-multiplexing contract [implementer_2: ran `cargo build` — missing crate errors for `tokio`, `futures-core`]") — not as an instruction. Entries must still be self-contained and reference no other agents, commands, conversations, or briefs: this section is the only surviving record once the spec is deleted, so it cannot rely on conversational context to be actionable later.
