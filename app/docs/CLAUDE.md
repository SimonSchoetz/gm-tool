...
The progress tracker is a sequencing reference read before starting work — not a live status document; do not modify it during implementation.
...
- ❌ `[FOUNDATION: SF2–SF4 depend on this. Stage as unit: same as SF3's Foundation annotation]` — delegation to another SF's file list is not permitted; every Foundation annotation must enumerate its own complete file list inline

### Key Architectural Decisions

Required section in every spec, placed after the progress tracker and before the first sub-feature section. Document every non-obvious structural choice the implementing instance needs to understand — data model shape, state ownership, persistence decisions, naming corrections, and anything the architecture implies but does not make explicit. Every annotation or stated-rationale requirement below exists because the spec is read by a fresh implementing instance with no conversational context — anything not written down here is invisible to that reader.

Each entry: a short heading stating the decision, followed by one paragraph of rationale. Cross-reference the CLAUDE.md rule that drove it when one applies. State decisions as facts — never attribute them to a conversation, a person, or a prior agent: provenance is noise to the reader this section's opening statement names.

**SF self-containment for architectural consequences.** When a KAD produces a structural choice in a specific SF that an implementer reading only that SF file would find surprising — a type from a foreign layer appearing as a parameter, a deliberate deviation from a boundary rule, or any choice whose rationale lives only in the root KAD section — the SF body must include a brief inline rationale note at the decision site, naming the KAD heading it corresponds to. The same inline-note requirement applies when a sub-feature's stated boundary (e.g. "do not modify X") knowingly walls off a violation discovered during spec-writing rather than a KAD-driven choice — the note is prefixed with the literal marker `[DEFERRED-VIOLATION: <rule + location>]` instead of a KAD heading, followed by the reason it is deferred rather than fixed now; the bracketed marker is required verbatim. An SF that contains a surprising structural choice, or a scope-excluding boundary concealing a known violation with no `[DEFERRED-VIOLATION]` note, is a spec defect, for the reason this section's opening statement gives.

**Untested residual risk annotation.** When spec-writer exempts a construct from unit tests under `app/src/CLAUDE.md`'s Testing Policy (a React component) and the risk falls in the category Testing Policy's Forbidden bullet excludes from that exemption, the SF body must enumerate every such risk, not only the first found, each as its own manual-verification note prefixed with the literal marker `[MANUAL-VERIFY]` at the start of the line, naming the exact interaction to verify by hand; the marker must appear verbatim. Separately, when a risk exists because a structural choice guards against a failure mode a future edit could reintroduce undetected, the SF body must also direct the implementer to add a named code comment at that construct stating the rationale and failure mode — never on the marker's own line. A construct meeting the trigger with no `[MANUAL-VERIFY]` note, or with only its first risk enumerated, is a spec defect, for the reason this section's opening statement gives.

**Third-party runtime testability verification.** Before accepting a Testing Policy exemption justified by a third-party runtime executing the construct, verify against the installed package tree (`node_modules`, `package.json`) whether that runtime is drivable headlessly without a component render. If drivable headlessly and the construct's directory falls within Testing Policy's scope (`helper/`, `/src/util/`), write the automated test instead of exempting it. If technically testable but outside that scope, do not default to `[MANUAL-VERIFY]` — record the scope gap in the CLAUDE.md impact section instead: a confirmed-testable construct with no owning test obligation is a Testing Policy gap, not a residual risk.
...
**Insertion anchor validity.** When a spec instructs the implementer to insert code at a named anchor line within an existing file (e.g., "add directly after `const [x, setX]`"), verify before writing that the anchor line appears after every dependency of the inserted declarations. If the inserted declarations reference a value declared below the proposed anchor, the anchor is wrong — state the correct anchor that appears after the last required dependency. This obligation is not conditional on the spec having named an anchor at all: when the inserted construct consumes a same-scope dependency synchronously — a direct read, or a closure over it invoked before render completes — the spec must state the placement constraint even if no anchor line is otherwise named. A closure whose invocation is deferred until after render completes (an effect, an event handler, any post-render callback) is not at risk regardless of textual order and does not trigger this obligation. A spec that names an anchor preceding a dependency, or that introduces a synchronously-consumed same-scope dependency with no placement constraint stated at all, is a spec defect, for the reason Key Architectural Decisions' opening statement gives.
...
This is a dependency order, not style: a layer may only reference what layers below it have already specified. Before writing each layer, cross-check the plan against conventions documented for that layer in the relevant scoped CLAUDE.md. A spec that omits a required pattern is incomplete, not just unimplemented.

**Cross-SF symbol lifecycle** — when a sub-feature introduces a symbol (a type field, exported constant, or any value) that has no consumer within the same sub-feature, the spec must explicitly name the later sub-feature that will wire it. A symbol with no current consumer and no forward reference is a spec defect, not a placeholder, for the reason Key Architectural Decisions' opening statement gives.
...
- **Reference-file substitution**: when a sub-feature's implementation can be fully derived from a named reference file plus a mechanical substitution table (e.g., a new domain entity following an established scaffold pattern), do not reproduce the file body in the spec. Name the reference file and state the substitution table. Before naming any file as a reference, validate it against current conventions per the shared rules file's Best Practices & Code Quality rule beginning "Never assume existing code is a compliant reference implementation..." — that rule binds at spec-writing time, not only at implementation time. If the candidate reference file has a violation, either name a different, compliant reference, or state the deviation explicitly in the spec's substitution table so the implementer does not copy it.
- **Original design with no reference file**: when a KAD establishes a structural decision (e.g., which module owns a piece of state, what a derived value's shape is), state the decision and its shape — not a full, near-final reproduction of what the decision implies, whether that reproduction takes the form of a function or component body, a CSS declaration block, or a prose paragraph that narrates the implementation step by step.

In both cases the spec must still fully specify: (1) any content that differs from a reference or that a decision does not fully determine, (2) the required assertion list for each test file (specific strings, error messages, SQL shapes), and (3) cross-SF consumer annotations. A spec that reproduces zero-decision content at full length — whether copied from a reference or freshly authored to look finished, regardless of medium — is not more complete.

### CLAUDE.md impact

Required section in every spec. Before marking a spec complete, evaluate:

- Do any CLAUDE.md files or `app/docs/_product/domain-scaffold.md` reference files, paths, or modules that this spec adds, renames, or removes — or enumerate a class of thing (a dependency, a command, a table, a structure-tree entry, a registered participant in an ambient or registration system) that this spec adds a new instance or new participant to? This includes adding a new consumer of an existing canonical or gating list already governed by a documented registration flow, not merely a type change.
...
- Does this spec introduce a genuinely new structural layer, ambient system, or convention that `app/docs/_product/domain-scaffold.md` does not yet describe at all — distinct from adding a new participant to an existing one, which the bullet above covers? If yes, update `app/docs/_product/domain-scaffold.md` with the new section.
...
- Did the Third-party runtime testability verification above find a construct that is confirmed testable but sits outside Testing Policy's scope? If yes, record the scope gap here as a stated fact plus its consequence, per the Epistemological Discipline citation format.

Entries whose target is `app/docs/_product/domain-scaffold.md` are the exception to this section's deferred-handoff treatment: it is not a CLAUDE.md file and has no automated instruction-refinement pathway to act on it — any bullet above that identifies a `domain-scaffold.md` gap must be resolved by editing it directly during this spec's own implementation, encoded as a concrete item in the relevant sub-feature's Files affected list (`Modified: app/docs/_product/domain-scaffold.md — <what changes>`), never logged here as a deferred fact. For each remaining impact found, state the affected CLAUDE.md file, the fact or structural change driving the update, and its citation per the Epistemological Discipline citation format. If there is no impact, write "None." This section is handoff material for a later instruction-refinement session, whatever runs it — never edit instructions for the implementing instance. Root CLAUDE.md bars direct CLAUDE.md edits by any role; phrasing an entry as a directive to apply ("add X to file Y") violates that rule even when the same instance would carry it out. Write each entry as a stated fact plus its consequence (e.g. "`app/src-tauri/CLAUDE.md`'s Dependencies list is missing `tokio` and `futures-core`, both required by SF1's stream-multiplexing contract [implementer_2: ran `cargo build` — missing crate errors for `tokio`, `futures-core`]") — not as an instruction. Entries must be self-contained, referencing no other agents, commands, conversations, or briefs — this section is the only surviving record once the spec is deleted.
...
The layered order rule above (Layered breakdown) applies inside each sub-feature file unchanged.

**CLAUDE.md impact assessment scope**: must cover the spec as a whole — evaluate all sub-features together before writing the impact entry.
