...
describe('update', () => {
  ...
  it('should update name and produce correct SQL', async () => {
    await update('test-id', { name: 'Updated Faction' });

    expect(mockExecute).toHaveBeenCalledWith(
      'UPDATE factions SET name = $1, updated_at = $2 WHERE id = $3',
      ['Updated Faction', '2024-01-15T10:30:00.000Z', 'test-id'],
    );
  });
  ...
  it('should update multiple fields', async () => {
    ...
    expect(mockExecute).toHaveBeenCalledWith(
      'UPDATE factions SET name = $1, summary = $2, updated_at = $3 WHERE id = $4',
