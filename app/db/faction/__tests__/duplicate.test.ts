...
vi.mock('../../util', async (importOriginal) => {
  ...
  return {
    ...actual,
    generateId: vi.fn(() => 'new-faction-id'),
  };
});
...
const INSERT_SQL =
  'INSERT INTO factions (id, adventure_id, summary, description, image_id, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7)';

describe('faction.duplicate', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('omits name so the duplicate has no name', async () => {
    await duplicate('source-faction-id', 'new-image-id');

    expect(mockExecute).toHaveBeenCalledWith(INSERT_SQL, expect.any(Array));
    expect(mockExecute).not.toHaveBeenCalledWith(
      expect.stringMatching(/^INSERT INTO factions \([^)]*\bname\b/),
      expect.anything(),
    );
  });
  ...
  it('writes null image id when passed null', async () => {
    await duplicate('source-faction-id', null);
  ...
  it('generates a fresh id and fresh timestamps', async () => {
    const result = await duplicate('source-faction-id', 'new-image-id');

    expect(result).toBe('new-faction-id');
    expect(mockExecute).toHaveBeenCalledWith(INSERT_SQL, [
      'new-faction-id',
      'adventure-123',
      'a dwarf merchant',
  ...
  it('throws when the source row does not exist', async () => {
    ...
    await expect(duplicate('missing-faction-id', null)).rejects.toThrow(
      'Faction not found: missing-faction-id',
    );
  });
  ...
  it('omits pinned_order so the duplicate starts unpinned', async () => {
    ...
    expect(mockExecute).not.toHaveBeenCalledWith(
      expect.stringMatching(/^INSERT INTO factions \([^)]*\bpinned_order\b/),
      expect.anything(),
    );
  });
});
