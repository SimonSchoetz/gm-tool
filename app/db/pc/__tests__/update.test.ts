...
describe('update', () => {
  ...
  it('should update name and produce correct SQL', async () => {
    await update('test-id', { name: 'Updated PC' });

    expect(mockExecute).toHaveBeenCalledWith(
      'UPDATE pcs SET name = $1, updated_at = $2 WHERE id = $3',
      ['Updated PC', '2024-01-15T10:30:00.000Z', 'test-id'],
    );
  });
  ...
  it('should update multiple fields', async () => {
    ...
    expect(mockExecute).toHaveBeenCalledWith(
      'UPDATE pcs SET name = $1, summary = $2, updated_at = $3 WHERE id = $4',
