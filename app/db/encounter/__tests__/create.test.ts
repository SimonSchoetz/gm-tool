...
describe('create', () => {
  ...
  it('should insert encounter and return generated ID', async () => {
    const encounterId = await create('adventure-123');

    expect(mockExecute).toHaveBeenCalledWith(
      expect.stringContaining('INSERT INTO encounters'),
      expect.arrayContaining(['test-generated-id']),
    );
    expect(encounterId).toBe('test-generated-id');
  });

  it('should set adventure_id, default name, and ISO timestamps', async () => {
    ...
    expect(mockExecute).toHaveBeenCalledWith(
      ...
      [
        'test-generated-id',
        'adventure-123',
        expect.stringMatching(/^New Encounter /),
