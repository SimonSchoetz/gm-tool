...
const INSERT_SQL =
  'INSERT INTO items (id, adventure_id, summary, description, image_id, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7)';

describe('item.duplicate', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('omits name so the duplicate has no name', async () => {
    await duplicate('source-item-id', 'new-image-id');

    expect(mockExecute).toHaveBeenCalledWith(INSERT_SQL, expect.any(Array));
    expect(mockExecute).not.toHaveBeenCalledWith(
      expect.stringMatching(/^INSERT INTO items \([^)]*\bname\b/),
      expect.anything(),
    );
  });
  ...
  it('writes null image id when passed null', async () => {
    await duplicate('source-item-id', null);
  ...
  it('generates a fresh id and fresh timestamps', async () => {
    const result = await duplicate('source-item-id', 'new-image-id');
  ...
  it('throws when the source row does not exist', async () => {
    ...
    await expect(duplicate('missing-item-id', null)).rejects.toThrow(
      'Item not found: missing-item-id',
    );
  });
  ...
  it('omits pinned_order so the duplicate starts unpinned', async () => {
    ...
    expect(mockExecute).not.toHaveBeenCalledWith(
      expect.stringMatching(/^INSERT INTO items \([^)]*\bpinned_order\b/),
      expect.anything(),
    );
  });
});
