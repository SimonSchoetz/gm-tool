...
- **Forbidden**: React components — files whose exported function returns JSX — must not have unit tests. Components change shape frequently; testing helpers and the data layer provides sufficient coverage of logic-level correctness at lower cost — but not of interaction behavior tied to the browser's native default action (e.g. `preventDefault()` handling in a keyboard/pointer handler), which requires a live DOM event dispatch no helper or data-layer test provides.
