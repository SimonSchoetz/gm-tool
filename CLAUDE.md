...
- **Re-derive types after every refactor**: After any refactor, audit every exported symbol in the changed files — types, factory functions, error constructors, constants — bottom-up from actual usage. Never trust existing definitions at face value. A symbol with no call site is dead code regardless of whether it is a type, a prop, or an exported function.
  ...
  2. Trace every field in internal types to a place where it is read. If a field is defined but never accessed, remove it — unless another CLAUDE.md rule independently mandates the field's presence regardless of current consumption. In that case retain it, and make the non-consumption explicit at the call site (destructure it with an underscore-prefixed alias and a comment naming the mandating rule) rather than silently discarding the return value.
...
- **Grep for duplicate raw literals or expressions before declaring any change set complete.** A raw literal — a regex, a semantic mapping, a magic string or number expressing a domain constraint — or a duplicated expression — a conditional, method chain, or multi-step derivation copied verbatim across call sites to compute the same value — that appears independently in two or more files is a missed DRY extraction, regardless of whether the change is a bug fix or new code (e.g. a validation regex re-declared across schema files, or a spec instructing a derivation expression already used elsewhere). Before considering any change set done, grep for every other raw usage of a literal or expression you just introduced, modified, or instructed; two or more independent instances must be extracted to one canonical source with every call site updated, not left as parallel copies.
...
- **Read discipline: read immediately before editing, and only what the current output depends on.** Treat prior read state as lost after context compaction — re-read before each edit, but never re-read after editing to verify the write; trust the edit result. Every read must be tied to a specific, named file or claim the current output depends on — if you cannot state which output line a read resolves before making it, do not make it; do not read to reconstruct historical context, traverse import graphs for ambient understanding, or confirm a write just performed. The pre-edit gate applies only to writing roles — read-only roles have no edits to gate and must not apply it as a defensive habit; role-specific scope ceilings are defined in each agent's file.
...
- **All automated checks must pass with zero errors or failures before any commit, and baseline failures are triaged by category before any implementation begins.** Between sub-features during implementation, run `npx tsc --noEmit`, `npx eslint .`, and `prettier --check .` — vitest produces noise from intentionally incomplete intermediate states at sub-feature boundaries and is the only check deferred. The full TypeScript suite (tsc + vitest + eslint + prettier) runs twice per session: at the start (baseline) and after the final review cycle concludes (before committing). When any `src-tauri/` file is touched, the Rust suite (clippy + fmt-check) also runs at both points. Checks and their invocations:
  ...
  Baseline failures fall into two categories: Minor — mechanical to fix and requiring no design judgment — fix and commit autonomously (`chore(<branch>): fix pre-existing test fixture errors`) without surfacing. Major — requiring a choice between valid alternatives, or with a cause ambiguous from the error output alone — surface to the user with a proposed fix before applying. Never treat any pre-existing error, minor or major, as acceptable baseline noise to filter out or defer.
