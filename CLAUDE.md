...
- **Never introduce manual line breaks in prose paragraphs or bullet list items in instruction files** (`.claude/agents/*.md`, `.claude/commands/*.md`, and all CLAUDE.md files) **or in code comments anywhere in the codebase.** Each paragraph, each bullet item, and each comment is one continuous line — visual wrapping is the IDE/renderer's responsibility. This does not apply to code blocks, tables, or fenced examples. Note: unlike Markdown, code comments are not universally soft-wrapped by every consumer (raw diffs, terminal output, some viewers render them unwrapped) — this is an accepted tradeoff, not an oversight.
...
- **Before wiring a prop to any component you did not write in the current task, read its implementation file and verify the prop is forwarded to the element or sub-component where it takes effect.** A prop declared in a component's props type may not be forwarded internally — TypeScript types describe the interface surface, not the internal wiring. Passing a prop to a component that accepts it but silently drops it is a runtime no-op that produces no compiler or linter error. This verification is required before writing the JSX; do not defer it to code review.
...
When a conversation surfaces a corrected understanding of a rule, convention, or approach — whether through user correction, a retrospective discussion, or independent self-diagnosis — apply the corrected version for the remainder of the current conversation immediately, without waiting for CLAUDE.md to be edited or for a new instance to read the change. The correction is binding the moment it is reached, not the moment it is written down. Do not revert to the prior, now-known-wrong approach later in the same session, and do not wait to be told again before applying it to the next applicable decision.

This applies regardless of how the correction originated — being caught missing a requirement (see Accountability on Missed Requirements above), a collaborative discussion that reaches a better approach, or recognizing the issue unprompted. Accountability on Missed Requirements governs explaining what went wrong; this rule governs behaving differently starting immediately after.
...
- **Grep all call sites before declaring a semantic-mapping fix complete.** When a bug fix touches a semantic mapping or lookup pattern (e.g., a direction-to-type or key-to-value table expressed as raw literals), grep the codebase for every other raw usage of that same mapping before considering the fix done — fixing the reported call site does not guarantee every call site was fixed. If the grep surfaces two or more independent raw-literal instances of the same mapping, that is itself evidence of a missed DRY extraction (see DRY above and Constants — Trigger 2 in app/src/CLAUDE.md): extract the mapping to one canonical source and update every call site to reference it, rather than patching each instance individually. This differs from "Re-derive types after every refactor" above: that rule audits exported symbols for dead code after an intentional refactor; this rule verifies bug-fix completeness across raw-literal duplicates that were never extracted as a symbol in the first place.
  - ❌ BAD: Fixing a `'top'`/`'bottom'` → column, `'left'`/`'right'` → row direction mapping in one file, without grepping for the same raw-literal mapping duplicated elsewhere — the second unsynchronized copy remains broken
  - ✅ GOOD: Grep confirms two raw-literal copies of the direction mapping exist → extract to a single exported `directionToAxis` lookup in `helper/`, update both call sites to import it
...
**Before verifying any external-system fact, check the knowledge base first.** The `.claude/knowledge/` directory caches verified facts about external systems, keyed by category file (e.g., `tauri.md`, `tanstack-query.md`). Step 0 of any verification is: read the relevant category file and check whether the fact is already recorded at the current version. If it is, treat it as established — no further lookup needed. If it is absent or the recorded version does not match the current installed version, proceed with verification and then write the result to the knowledge base.

**Every agent that verifies an external-system fact must write it to the knowledge base.** After completing any verification — reading documentation, inspecting type declarations, running toolchain commands — write the result to `.claude/knowledge/<category>.md` using the canonical entry format defined in `.claude/knowledge/CLAUDE.md`. This obligation applies to all agents with Write or Edit permission. If Write/Edit permission is absent, surface the unrecorded fact to the caller so it can be persisted by an agent with permission.

**Knowledge base entries are invalidated by version mismatch.** When the recorded version in a knowledge base entry does not match the currently installed version (from `package.json`, a schema URL, or equivalent), the entry is stale. Re-verify the fact against the current version, then append a `**Reverified at:**` block to the existing entry — never overwrite. The stale entry remains as a historical record.

**Apply the same error-correction obligation to knowledge base entries as to files you touch.** When reading a knowledge base entry and finding it incorrect — the fact is wrong, the citation is broken, or the version field is malformed — correct it in the same pass. Do not leave a known-incorrect entry in place. This mirrors the "fix violations in files you touch" rule and applies to all agents with Write or Edit permission.
...
- **Code in any artifact must be valid under the project's full toolchain configuration, not just type-declaration correct.** Before including a code example in a spec, brief, review finding, or architectural decision, verify it is compatible with the active `tsconfig.json` compiler flags and ESLint plugin rules — not merely that the symbols exist. Three classes of error pass symbol verification but fail at toolchain application time: (1) general tsc type errors — e.g., narrowing a method-call expression across two invocations (`$isLinkNode(node.getParent()) ? node.getParent() : null`) fails because tsc narrows variables, not re-evaluated call expressions; store the result in a `const` first; (2) compiler flag strictness — e.g., with `exactOptionalPropertyTypes: true`, assigning `T | undefined` to an optional property typed `T` is a type error even though both types are valid in isolation; (3) ESLint plugin rules — e.g., `react-hooks/refs` bans reading `ref.current` inside a render callback even when the ref is correctly typed. Verify against `app/tsconfig.json` and `app/eslint.config.js` before writing code that depends on their settings. Training knowledge of what TypeScript or ESLint permits in the abstract is never sufficient.
...
- **Distinguish one-shot workers from long-running teammates.** One-shot workers execute, return a result, and exit — they cannot receive further messages and must not be "resumed." Long-running teammates persist across turns and can receive further messages. Do not assume this distinction, or the current mechanism for spawning and addressing each kind, from training knowledge or from a prior session's record in this file — verify against the live tool registry in the current context window before asserting how spawning or addressing works. If a command or prior instruction names a tool that is absent from the active tool list, do not treat the absence as a signal to fall back to a different approach without first checking whether the tool must be loaded (e.g. via a tool-search mechanism) versus no longer existing at all — an absent tool can mean either, and the two require different corrections. At the time of writing, the working mechanism is: spawn a long-running teammate via the `Agent` tool's `name` parameter, address it via `SendMessage({to: name})`; treat this as illustrative, not authoritative — re-verify if the tool registry has changed since. Before spawning a new teammate for the same role, attempt to resume the existing one via a message. If that fails after one retry, surface the failure explicitly — never silently substitute a new instance. Silent substitution hides continuity loss from the user and invalidates any prior context the teammate held.
...
- **Never edit any CLAUDE.md file directly, and never present a CLAUDE.md edit as an option for the user to choose.** This prohibition covers both mechanisms: (1) writing or editing a CLAUDE.md file directly, and (2) offering a CLAUDE.md change as a selectable choice via `AskUserQuestion` or any other choice-construction — even when the user would be the one to accept it. Route every CLAUDE.md instruction gap through `/refine-claude` instead. This applies regardless of role or how minor the change appears — CLAUDE.md changes are head-of-instructions' and head-of-agents' domain exclusively, mediated through the `/refine-claude` retrospective.
