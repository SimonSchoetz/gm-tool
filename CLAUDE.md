...
- **Git command discipline.** Never prefix commands with `cd` — the working directory is already correct in Claude Code's process context, and `cd /path && git ...` breaks `Bash(git *)` permission matching. Never stage by directory path — enumerate every file explicitly (`git add <file1> <file2> ...`, never `git add <directory>/`), since a directory path silently skips sibling files at adjacent levels belonging to the same logical change. Stage each file only as the last action before the commit that includes it — never before further edits to it. Staging early (e.g. via `git mv`) then editing leaves the index stale; no toolchain check reads the index, only the working tree.
