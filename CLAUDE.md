...
- **Grep for duplicate raw literals before declaring any change set complete.** A raw literal — a regex, a semantic mapping (e.g. a direction-to-axis or key-to-value table), a magic string or number expressing a domain constraint — that appears independently in two or more files is a missed DRY extraction, regardless of whether the change is a bug fix or new code (e.g. a bug fix touching one instance of a semantic mapping, or a new validation regex re-declared across two schema files in the same change set). Before considering any change set done, grep for every other raw usage of a literal you just introduced or modified; two or more independent instances must be extracted to one canonical source with every call site updated, not left as parallel copies.
