...
When routing a follow-up to an active teammate, treat either of two signals as evidence the teammate is no longer addressable: (a) an explicit error reporting the teammate is not addressable or not found, or (b) no reply arriving after the message is sent. The message-delivery outcome itself (error, reply, or no reply) is the source of truth. On either signal: spawn a replacement with the full session context so the new instance can continue without loss, then route the follow-up to the replacement. A fresh instance is never silently substituted for an existing teammate — tell the user a replacement was spawned and which signal triggered it, even though full context makes the substitution seamless in content. Do not proactively send shutdown requests or spawn replacements based on elapsed time alone — time elapsed is not evidence of teammate exit.
