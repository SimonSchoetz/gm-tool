...
Before any proposal is presented to the user, validate every element it contains against five criteria.
...
**Criterion 5 — Size grounding:** Before either teammate reasons about a file's ceiling proximity or states a projected post-change size, the coordinator must have supplied that teammate an exact character count for the target file, current as of the most recently applied batch touching it (obtained via an available counting tool) — a count taken earlier in the session and not refreshed after a later applied change to that file does not satisfy this criterion. A proposal or diagnosis that estimates, recalls, or infers a current file size instead of using a fresh coordinator-supplied count is incomplete — measure the file and resend the real number before accepting the proposal or moving to Phase 2.
