...
- if they are related to the current spec (e.g. the specs, changes to this project's knowledge store (resolved via root CLAUDE.md's Epistemological Discipline → Knowledge base artifact-key)), commit them.
...
3. Run each baseline check independently — never chain them, so a `type-check` failure can't short-circuit `lint`, `format-check`, or `test`: run the `type-check`, `lint`, and `format-check` rows of root CLAUDE.md's Tool Use Discipline → Check Commands, each in its own stated Working directory. Then run the `test` row. If the spec's "Files affected" sections name a path matching the `rust-lint`/`rust-format-check` rows' Trigger, also run those two rows, independently of the JS checks and each other.
...
2. Run the `type-check` row of Check Commands (root CLAUDE.md — Tool Use Discipline). Resolve every error before continuing.
3. Run the `lint` row of Check Commands. Resolve every error before continuing.
4. Run the `format-check` row of Check Commands. Resolve every formatting error before continuing — independently of `type-check` and `lint`, per step 3 above.
5. Do not run the `test`-cadence or `rust-lint`/`rust-format-check` rows between sub-features — intermediate states produce failures that are not yet meaningful; those run only at the baseline and pre-review checkpoints, per their Cadence in Check Commands.
...
0. Run every row of Check Commands (root CLAUDE.md — Tool Use Discipline) whose Trigger condition holds, per its own Cadence and Working directory, and resolve any errors or failures — this is the full suite: every triggered row, regardless of Cadence. The reviewer must see code that passes every row that applies before filing findings. Before citing any CLI flag or subcommand for a Check Commands row in this file, verify it against root CLAUDE.md's current Invocation — never state a flag from memory.
...
After any type, prop, or function signature change, run the `type-check` row of Check Commands (root CLAUDE.md — Tool Use Discipline) before touching any call sites. The compiler output is the authoritative list of what needs updating — do not rely on memory or manual search.
