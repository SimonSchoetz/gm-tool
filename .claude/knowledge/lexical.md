...
## `LexicalEditor.setRootElement(nextRootElement)` wipes the previous root element's DOM children via `textContent = ''` before firing `registerRootListener` callbacks — a React portal targeting the root element cannot react in time to unmount itself first

**Verified at:** lexical 0.46.0
**Citation:** [implement_1: app/node_modules/lexical/dist/Lexical.dev.mjs:14495-14552 — `setRootElement` calls `resetEditor(this, prevRootElement, nextRootElement, ...)` at line 14501, then `triggerListeners('root', ...)` at line 14550 (after); :13820-13859 — `resetEditor` sets `prevRootElement.textContent = ''` at line 13845, unconditionally and synchronously, before any root listener has run; app/node_modules/@lexical/react/dist/LexicalContentEditable.dev.mjs:82-90 — `ContentEditable`'s DOM `ref` callback calls `editor.setRootElement(null)` on unmount, meaning the wipe happens as part of React's ref-cleanup step]
...
## `menuRef`/`targetLineRef` root elements must use `position: absolute; left: 0; top: 0;` as their base CSS, never `left: -10000px; top: -10000px;` — Lexical's `translate()` math is additive to this base, not a replacement for it

**Verified at:** @lexical/react 0.46.0

**Citation:** [spec-writer_14: app/node_modules/@lexical/react/dist/LexicalDraggableBlockPlugin.dev.mjs:214-227 (`setMenuPosition` computes `top = (targetRect.top + ... - anchorElementRect.top + anchorElem.scrollTop) / zoomLevel` — an offset measured from `anchorElem`'s own origin, with no term correcting for any pre-existing CSS `left`/`top` on the floating element — then sets only `.style.display`, `.style.opacity`, `.style.transform`); :245-272 (`setTargetLine` computes `top`/`left` the same way, sets only `.style.transform`/`.style.width`/`.style.opacity`); :274-279 (`hideTargetLine` hides via `transform: translate(-10000px, -10000px)`, itself relying on a zero CSS base to land off-screen); empirically confirmed via a diagnostic build in this repo: `.block-drag-handle` given a CSS base of `left: -10000px; top: -10000px;` received a real, non-default inline `transform: translate(4px, 694px)` from a genuine hover (proving the JS positioning logic ran correctly), yet was not visible on screen; forcing the element to `position: fixed; top: 50px; left: 50px;` via a devtools override made it visible immediately, isolating the defect to the CSS base conflicting with the additive transform, not to paint/z-index/visibility]

CSS `transform: translate(X, Y)` is always relative to the element's own already-laid-out position — for a `position: absolute` element that position is exactly its `left`/`top`. Lexical's own `top`/`left` math computes the desired offset **from `anchorElem`'s origin**, assuming the floating element's pre-transform position is `(0, 0)` relative to that origin; it never compensates for a nonzero CSS `left`/`top`. A stylesheet declaring `left: -10000px; top: -10000px;` as the "off-screen hidden" base therefore renders the shown state at approximately `(-10000 + X, -10000 + Y)`, which stays off-screen for any realistic `X`/`Y` — this is a previous, uncorrected entry in this file and is wrong; do not reintroduce it. The correct pattern: `position: absolute; left: 0; top: 0;` as the base, with the element hidden by other means — `display: none` (for the `menuRef` root, matching `setMenuPosition`'s own `display: none` branch when no target is hovered) or `opacity: 0` (for the `targetLineRef` root, matching `hideTargetLine`'s `-10000px` transform-based hide, which itself only lands off-screen correctly when the CSS base is zero). A consumer's stylesheet may add any other visual property (border-radius, cursor, hover color, height/background) but must never declare its own `transform`, which Lexical overwrites every frame, nor a nonzero `left`/`top`, which corrupts every subsequent `translate()` Lexical computes.
...
## `anchorElem` must never be `editor.getRootElement()` — Lexical registers its own `MutationObserver` on the root element and reconciles away any DOM child it doesn't recognize, including a portal's own nodes, almost immediately after insertion

**Verified at:** lexical 0.46.0

**Citation:** [implement_2: app/node_modules/lexical/dist/Lexical.dev.mjs:1178-1183 — `initMutationObserver(editor)` creates `editor._observer = new MutationObserver((mutations, observer) => flushMutations(editor, mutations, observer))`; :14522 — `setRootElement` calls `initMutationObserver(this)` on the new root element; :1042 — `flushMutations` is the observer callback, reached via `$beginUpdate` → `updateEditorSync`; empirically confirmed via a diagnostic build in this repo: intercepting `Node.prototype.removeChild` and logging every call targeting `DraggableBlockPlugin_EXPERIMENTAL`'s portaled wrapper/drop-indicator divs showed `matched=true` (a legitimate, non-mismatched removal) with a stack trace `Node.removeChild → Lexical.dev.mjs:1102 → $beginUpdate → updateEditorSync → flushMutations → MutationObserver callback`, firing on the same synchronous tick as the portal's insertion via `commitPlacement`]

`DraggableBlockPlugin_EXPERIMENTAL` (and any other consumer using `createPortal(..., anchorElem)`) must receive an `anchorElem` that Lexical does not itself manage as a root element — this is why the plugin's own default is `document.body` [see `app/node_modules/@lexical/react/src/LexicalDraggableBlockPlugin.tsx:603` — `anchorElem = document.body`], never `editor.getRootElement()`. Passing the Lexical root element (the `contentEditable` node, e.g. `.editor-content`) as `anchorElem` causes Lexical's own root-scoped `MutationObserver` to observe the portal's `insertBefore`/`appendChild` calls as an unrecognized DOM mutation and reconcile it away via `flushMutations`, removing the portaled nodes on the same tick they were inserted — before they are ever painted, and independent of component unmount order entirely. This fully explains a `removeChild NotFoundError` on later unmount (the nodes are already gone) and a portal that silently never renders (the earlier symptom): the fix is to render the portal into a plain, React-owned ancestor element outside Lexical's root-element management — e.g. a wrapper `<div>` around `<ContentEditable>` that the consuming component also renders, obtained via a normal ref/state pair, not `editor.registerRootListener`. This is a more complete explanation than — not a contradiction of — the entry above ("`LexicalEditor.setRootElement(nextRootElement)` wipes the previous root element's DOM children..."): that entry's `textContent = ''` unmount-wipe mechanism is real and independently verified, but for this specific `anchorElem`-as-root-element bug it was never reached, because the MutationObserver reconciliation already removed the nodes long before any unmount occurred.
