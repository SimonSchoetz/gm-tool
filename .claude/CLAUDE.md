...
`.claude/knowledge/` is this project's knowledge store — verified external-system facts, tracked in this repo and used per root `CLAUDE.md`'s Epistemological Discipline → Knowledge base. `.claude/retro-log.md` moved to `~/dev/setup`, project-scoped under this project's `artifact-key` (declared in that same section): `~/dev/setup/claude/projects/gm-tool/retro-log.md`, with its format authority at `~/dev/setup/claude/devloop/CLAUDE.md`. `.claude/reference/` moved to `~/dev/setup/claude/devloop/reference/` too, but shared across every project using the plugin rather than project-scoped — it holds process content, not project facts.
