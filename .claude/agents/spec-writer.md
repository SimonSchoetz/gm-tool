...
- After verifying any external-system fact (WebFetch, type declarations, toolchain runs) — including the third-party library check above — apply the write obligation from root CLAUDE.md: write it to `.claude/knowledge/<topic>.md` before completing the verification task, per `.claude/knowledge/CLAUDE.md`'s Write Obligation. If Write permission is absent, record the fact inline in the chat response prefixed `[KNOWLEDGE PENDING WRITE: <topic>]`.
