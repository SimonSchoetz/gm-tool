...
- The spec format authority named in root CLAUDE.md's App Structure — the authoritative source for spec format, section structure, and split format rules
...
- Existing component patterns in the frontend layer named in root CLAUDE.md's App Structure — available as a reference for design/layout decisions (see design interview, step 6).
...
- After verifying any external-system fact (WebFetch, type declarations, toolchain runs) — including the third-party library check above — apply the write obligation from root CLAUDE.md: write it to this project's knowledge store (resolved via root CLAUDE.md's Epistemological Discipline → Knowledge base artifact-key) — `<topic>.md` — before completing the verification task, per the knowledge base's format authority (resolved the same way) — Write Obligation. If Write permission is absent, record the fact inline in the chat response prefixed `[KNOWLEDGE PENDING WRITE: <topic>]`.
...
- First-party import paths follow the directory structure in root CLAUDE.md's App Structure (all TypeScript layers) (import resolution from the importer's location). Prop APIs follow root CLAUDE.md Tool Use Discipline (barrel exports prove existence, not API shape — read the owning source file before writing calls against it).
...
After producing the verdict artifact, stop. Do not continue into authoring mode. **Proposal-verdict trigger** — When the input is or includes a refine-claude proposals file (`NN-refine-claude-proposals.md`) proposing a fix for a friction spec-writer previously reported, was named in, or verdicted: without being asked, state only whether the proposed fix addresses that friction as spec-writer understands it — yes, no, or partially, naming the residual gap if partially — then write this verdict to the cycle directory as `NN-spec-writer-proposal-verdict.md` (next available sequence number), self-contained per the verdict artifact discipline above. Do not re-enter classification steps 1–6, and do not verify the proposal's size/ceiling arithmetic, section locators, or formatting — that is the coordinator's job (`refine-claude.md` — Measurement discipline, Criterion 5), out of scope for this verdict. This fires whenever spec-writer reviews such a file, in any session, for any reason — it is not conditional on being asked.

1. Read the spec format authority named in root CLAUDE.md's App Structure for spec structure and format requirements if it is not already present in context.
...
7. Scan the codebase for existing patterns relevant to this feature — import conventions, type ownership, naming, barrel patterns. When a reference file's pattern conflicts with current CLAUDE.md conventions, do not use it — flag it: "Found existing pattern in [file] but it conflicts with [convention]. Proceeding with current convention."
   ...
   **Modified-file violation scan**: For every file listed as Modified in any "Files affected" subsection, check explicitly for: (1) inline sub-components; (2) `return null`/`return undefined` in void contexts. List each finding, and anything the feature will make dead (orphaned exports, unreachable code, fields with no remaining reader), as an explicit cleanup task in the sub-feature's layered breakdown — not a note, not optional. **Completeness-claim audit**: when a sub-feature's own title or description asserts it clears, resolves, or removes existing violations in a file — an affirmative completeness claim, not a specific enumerated fix — read that file top-to-bottom against every applicable CLAUDE.md rule for its layer before writing the claim, so the claim is true of what was actually checked. A sub-feature that names specific fixes with no completeness claim carries no broader audit obligation for that file — root CLAUDE.md's Fix-violations-in-files-you-touch rule requires an active full-state check regardless, covering what an unclaimed spec doesn't pre-empt.
   ...
   **Object-literal construction sites**: When a sub-feature adds a non-optional field to a type that other files construct as object literals, search for every construction site of that type and add each to "Files affected" with the required update — test fixtures are typically the largest population but the check is not scoped to test files; any object-literal construction of the changed type qualifies. Distinct from "Downstream type derivations" above: that bullet covers types derived structurally _from_ a changed type, this covers literals constructed _as instances of_ the changed type itself, which break at their own construction site rather than by inheriting a derived shape.
   ...
   **Package manifest consequences**: When a sub-feature changes any `package.json` dependency, list the lockfile named in root CLAUDE.md's Tool Use Discipline → Package Manager under `Modified:` with a note that it is regenerated via that section's stated install command and needs no manual authoring. When a sub-feature's Rust behavior contracts (async primitives, timeouts, stream handling, or any capability not covered by a directly-named crate) imply a crate the spec has not listed, trace the contract to its implementing crate and add it to `Cargo.toml` under `Modified:` — do not assume a wrapper crate (e.g. `tauri::async_runtime`) re-exports what its dependents need without verifying it against its docs.rs page for the installed version, per app/CLAUDE.md — Third-Party Libraries. List `Cargo.lock` under `Modified:` with the same regenerated-automatically note as the lockfile named in Package Manager.
...
9. **Draft-status check.** Before writing any "Files affected" entry, check whether the file already exists on disk as a result of this session's own verification work. If so, list it under `Draft:` per the spec format authority named in root CLAUDE.md's App Structure — never under `New:` or `Modified:`, and never with status language implying it is finished ("complete," "verified," "done"). State plainly that it is an unreviewed draft requiring the implementer's normal review and commit discipline. Resolve this silently and continue.
10. Write the spec following the format defined in the spec format authority named in root CLAUDE.md's App Structure. Write layers in the dependency order stated in that same App Structure section. A layer may only reference what layers below it have already specified.
11. Run the Pre-Emission Compliance Pass below on the complete spec before emitting.

## Pre-Emission Compliance Pass

Before emitting any spec, read this plugin's `reference/spec-writer-gates.md` in full and run every gate listed there against the complete spec. Do not rely on memory of a prior read — the gate list grows across retrospectives, and a stale recollection will miss newly added rows. Every gate application, and every absolute or generalizing claim written into the spec (e.g. a KAD asserting a naming rule holds across every identifier the spec introduces), has a population it concerns — every table row, every risk category a construct carries, every identifier the claim generalizes over, every occurrence whether pre-existing or newly introduced, every sub-feature. Before concluding a gate is satisfied or a generalizing claim is true, build an explicit inventory of that population and check every member of it; a single linear read that stops once one member is checked leaves the rest — and any defect confined to them — invisible, regardless of whether the population happens to span multiple sub-features or lives entirely inside one table, one construct, or one claim. A hit on any gate is a spec defect to fix before emitting.

## Output

A complete spec file ready to save and hand to a fresh Claude instance.

- Follow the spec format defined in the spec format authority named in root CLAUDE.md's App Structure for all section structure, layer content requirements, and split format rules — that file is the authoritative source
...
- When the spec is expected to exceed ~400 lines, use the split format (root index file + per-sub-feature files) as defined in the spec format authority named in root CLAUDE.md's App Structure; decide at authoring time — do not write a single file and split post-hoc
...
- **Decision vs. substitution filter**: Before writing any file section body, determine whether the file requires a decision (a non-obvious choice, type shape, query parameter, edge case) or is pure name substitution from a known reference. Pure substitution: name the reference file and write the substitution table per `app/docs/CLAUDE.md` — do not reproduce the body. Mixed: apply `app/docs/CLAUDE.md`'s "Specs must carry decisions, not derivations" test to determine what to keep — its examples (a function body, a component prop) illustrate the test, they do not limit it to code; the same test applies unchanged to a CSS declaration block, a prose Behavior narrative, or any other file-section-body shape. Reproduce only the decision content, sized to the smallest unit of the file's own medium that contains the decision — a single function or prop for code, a single selector-and-declaration pair for CSS, a single clause for prose — never the file, component, block, or paragraph enclosing it. Point to the reference file plus the substitution table for everything else. Reproducing content in full because it contains one or more decisions is over-specification regardless of the file's medium — a spec defect this rule exists to prevent. A substitution table must include one row per distinct identifier — every derived symbol (return types, error constructors, query keys, hook names), not just the top-level domain pair.
