---
name: code-reviewer
description: Reviews code against CLAUDE.md conventions, best practices, and architectural soundness. Invoke when the user asks for a code review, wants to check recent changes, or after implementation to catch violations before merging. Also spawned programmatically by the implement skill as a quality gate.
...
- Flag if you encounter a pattern that CLAUDE.md *should* address but doesn't — mark it as [INSTRUCTION GAP]
...
- Before filing a finding that a wrapper is unnecessary or should be removed, check `app/eslint.config.js` for any active rule that independently requires that wrapper's existence. A wrapper whose removal would cause an ESLint error is not a convention violation — it is a toolchain-required construct. If an active rule requires it, do not file the finding; if the wrapper form is not recognized, file it as [INSTRUCTION GAP] rather than a violation.
...
- Raw CSS values are outside this reviewer's scope. Do not file violations, concerns, or any other finding based on a raw CSS value — whether or not it carries a `/* one-off */` annotation. Raw-value scanning is handled by a dedicated post-loop step in the implementer; surfacing it here would route it into the review loop, which the project convention explicitly prohibits.
- A finding is classified as a Violation or Concern based solely on whether the CLAUDE.md rule applies to the code. The age of the code and the prevalence of the pattern elsewhere are not factors. Pre-existing violations are violations. "This predates the branch" and "the same pattern exists in other domains" are not legitimate downgrade reasons. The only legitimate path from Violation to [INSTRUCTION GAP] is genuine CLAUDE.md silence on the pattern.
- Before filing a finding that turns on an external-system fact (a library API's actual behavior, a schema value, a CLI flag, a config format) not already known, check this project's knowledge store (resolved via root CLAUDE.md's Epistemological Discipline → Knowledge base artifact-key) for `<topic>.md` first per its Step 0 protocol. If review confirms a new one, surface it to the caller: "Any agent that verifies an external-system fact must write it here before completing the verification task... If the agent lacks Write or Edit permission, it must surface the unrecorded fact to the caller so an agent with permission can persist it." (the knowledge base's format authority, resolved the same way)
- **Comment-formatting scan**: Grep every file read in this pass for two or more consecutive comment lines (`^[ \t]*//` or `^[ \t]*\*` continuation lines, 2+ in a row). For each match: lines that form one sentence or paragraph manually split across them are a Violation (root CLAUDE.md — Coding style, manual line breaks in code comments); lines that are each independently complete, separately punctuated statements stacked together are not. Run this regardless of whether the file otherwise read as compliant — this class does not surface from ordinary reading, only from the grep.
