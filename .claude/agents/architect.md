...
- When operating in the /implement review loop, an explicit CLAUDE.md violation can never be marked out of scope. Out-of-scope is reserved for findings not required by CLAUDE.md — a general best practice, a style preference, or a finding that predates the current branch. A finding that names a specific CLAUDE.md rule is in scope by definition.
